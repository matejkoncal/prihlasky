...
function SummaryMetric({ label, value, accent = false }: { label: string; value: string; accent?: boolean }) {
	return (
		<Box sx={{ minWidth: 76, px: 1.5, borderLeft: "1px solid", borderColor: "divider", "&:first-of-type": { borderLeft: 0 } }}>
			<Typography
				variant="caption"
				sx={{ display: "block", color: "text.secondary", fontSize: 10.5, fontWeight: 700, letterSpacing: 0.45, lineHeight: 1.2, mb: 0.35 }}
			>
				{label}
			</Typography>
			<Typography sx={{ fontSize: 17, lineHeight: 1.15, fontWeight: 800, color: accent ? "primary.main" : "text.primary" }}>{value}</Typography>
		</Box>
	);
}
		...
		<Container maxWidth="lg" sx={{ py: { xs: 3, md: 5 } }}>
			...
			{applications.length > 0 && (
				<Paper
					...
					variant="outlined"
					sx={{ overflow: "hidden", borderRadius: 3, borderColor: "#d9e3ec", boxShadow: "0 10px 35px rgba(16,47,74,.055)" }}
				>
					{applications.map(application => {
						...
						const status =
							summary.criterion === "met"
								? { label: "Kritérium splnené", color: "success" as const }
								: summary.criterion === "not-met"
									? { label: "Kritérium nesplnené", color: "error" as const }
									: assigned === 0
										? { label: "Čaká na pridelenie", color: "default" as const }
										: summary.completedCount === 0
											? { label: "Čaká na hodnotenie", color: "info" as const }
											: { label: "Hodnotenie prebieha", color: "info" as const };
						return (
							<Box
								key={application.id}
								sx={{
								...
								<Box
									...
									sx={{
										...
										gridTemplateColumns: {
											...
											lg: "46px minmax(250px,1fr) minmax(190px,auto) auto 48px",
										},
										alignItems: "center",
										columnGap: { xs: 1.25, md: 2 },
										rowGap: 1.25,
										borderLeft: "3px solid",
										borderLeftColor: summary.criterion === "met" ? "success.main" : summary.criterion === "not-met" ? "error.main" : "transparent",
										bgcolor: expanded ? "rgba(23, 105, 170, .045)" : "background.paper",
										transition: "background-color .18s ease",
										"&:hover": { bgcolor: expanded ? "rgba(23, 105, 170, .06)" : "#fbfdff" },
									}}
								>
									<Box
										sx={{
											...
											gridRow: 1,
											borderRadius: 2,
											bgcolor: summary.isComplete ? "rgba(46,125,50,.1)" : "#eaf2f8",
											color: summary.isComplete ? "success.dark" : "#255777",
											fontSize: 13,
											fontWeight: 850,
										}}
									>
										{applicantInitials(application.applicant_name)}
									</Box>
									<Box data-testid="application-identity" sx={{ minWidth: 0, gridColumn: 2, gridRow: 1 }}>
										<Typography variant="h6" noWrap sx={{ fontSize: 17, fontWeight: 800, lineHeight: 1.25 }}>
											{application.applicant_name}
										</Typography>
										<Typography variant="body2" noWrap color="text.secondary" sx={{ mt: 0.35 }}>
									...
									>
										<Chip
											...
											label={status.label}
											color={status.color}
											variant={summary.criterion === "pending" ? "outlined" : "filled"}
											sx={{ height: 27, fontWeight: 750 }}
										/>
										{summary.isComplete && summary.categoryCount === 5 && (
											<Tooltip title="Exportovať hodnotenie PDF">
												<IconButton
													...
													sx={{ border: "1px solid", borderColor: "rgba(46,125,50,.28)", borderRadius: 1.75 }}
												>
									...
									<Stack
										...
										direction="row"
										spacing={0.5}
										sx={{ justifySelf: "end", gridColumn: { xs: 3, lg: 5 }, gridRow: 1 }}
									>
										<Tooltip title={expanded ? "Skryť detail" : "Zobraziť detail"}>
											<IconButton
												...
												sx={{
													border: "1px solid",
													borderColor: expanded ? "primary.main" : "#d9e3ec",
													bgcolor: expanded ? "rgba(25, 118, 210, .09)" : "#f8fafc",
												}}
											>
