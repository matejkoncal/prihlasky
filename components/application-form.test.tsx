...
const PRIVACY_LABEL = "Súhlasím so spracovaním osobných údajov uvedených v tejto prihláške na účely výberového konania projektu Erasmus+.";
...
describe("ApplicationForm", () => {
	...
	it("renders the official co-funding mark", () => {
		render(<ApplicationForm />);
		expect(screen.getByRole("img", { name: "Spolufinancované Európskou úniou" })).toBeInTheDocument();
	});
	...
	it("rejects attachments larger than 3 MB in total without calling the API", async () => {
		...
		await user.upload(screen.getByLabelText("Motivačný list / Motivation letter"), second);
