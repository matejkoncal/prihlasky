...
export async function inviteReviewer(formData: FormData): Promise<{ error?: string }> {
	...
	if (
		...
		return { error: "Zadajte platný e-mail a meno hodnotiteľa" };
	}
	const session = await createServerSupabaseClient();
	const user = await getVerifiedStaffUser(session as unknown as StaffAuthClient);
	if (user?.role !== "admin") {
		return { error: "Nemáte oprávnenie správcu" };
	}
	if (role !== "reviewer" && role !== "admin") {
		return { error: "Vyberte rolu používateľa" };
	}
	const origin = (await headers()).get("origin") || process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
	const { data, error } = await createAdminSupabaseClient().auth.admin.inviteUserByEmail(email, {
		data: { display_name: displayName.trim() },
		redirectTo: new URL("/accept-invite", origin).toString(),
	});
	if (error) {
		console.error("Reviewer invitation failed:", error.message);
		return {
			error: error.message.includes("rate limit")
				? "Supabase dočasne obmedzil odosielanie e-mailov. Skúste to o niekoľko minút."
				: "Pozvánku sa nepodarilo odoslať. Používateľ už možno existuje.",
		};
	}
	if (!data.user) {
		return { error: "Supabase nevrátil pozvaného používateľa." };
	}
	const { error: roleError } = await createAdminSupabaseClient().from("profiles").update({ role }).eq("id", data.user.id);
	if (roleError) {
		return { error: "Pozvánka odišla, ale rolu sa nepodarilo nastaviť." };
	}
...
export async function deactivateStaff(formData: FormData): Promise<{ error?: string }> {
	...
	if (typeof profileId !== "string" || !/^[0-9a-f-]{36}$/i.test(profileId)) {
		return { error: "Neplatný používateľ" };
	}
	const session = await createServerSupabaseClient();
	const user = await getVerifiedStaffUser(session as unknown as StaffAuthClient);
	if (user?.role !== "admin") {
	...
	if (error) {
		return {
			error: error.message.includes("At least one")
				? "Nie je možné deaktivovať posledného aktívneho admina."
...
export async function reactivateStaff(formData: FormData): Promise<{ error?: string }> {
	...
	if (error) {
		return { error: "Používateľa sa nepodarilo aktivovať." };
	}
	revalidatePath("/admin/hodnotitelia");
	revalidatePath("/admin");
	return {};
