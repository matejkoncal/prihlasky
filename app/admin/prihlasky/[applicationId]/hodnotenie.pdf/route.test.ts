...
describe("admin evaluation PDF route", () => {
	it("enforces active admin authorization and valid ids", async () => {
		...
		expect(
			(await route.createEvaluationExportRoute(dependencies({ ...admin, role: "reviewer" as never }))(new Request("https://example.test"), context()))
				.status
		).toBe(403);
	...
	it("returns a generated PDF with download headers", async () => {
		...
		const response = await route.createEvaluationExportRoute(deps)(new Request("https://example.test"), context());
