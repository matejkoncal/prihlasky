...
type ExportLookup =
  | { status: "ok"; data: EvaluationExportData }
  | { status: "not_found" }
  | { status: "incomplete" };
...
export function createEvaluationExportRoute(dependencies: EvaluationExportDependencies) {
  return async function evaluationExport(
    _request: Request,
...
export const GET = createEvaluationExportRoute({
  ...
  async getExport(applicationId) {
    ...
    if (error) {
      if (error.message.includes("incomplete")) return { status: "incomplete" };
      if (error.message.includes("unavailable")) return { status: "not_found" };
