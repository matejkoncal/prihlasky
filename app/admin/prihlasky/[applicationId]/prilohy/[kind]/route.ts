...
export function createAttachmentDownloadRoute(dependencies: AttachmentDownloadDependencies) {
  return async function attachmentDownload(
    _request: Request,
    ...
    if (
      ...
      || (kind !== "cv" && kind !== "motivation_letter")
    ) {
    ...
    try {
      ...
      if (!metadata) return Response.json({ error: "Dokument sa nenašiel" }, { status: 404 });
      const file = await dependencies.download(metadata.storage_path);
      if (!file) return Response.json({ error: "Dokument sa nenašiel" }, { status: 404 });
