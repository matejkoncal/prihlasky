...
export async function submitEvaluation(formData: FormData) {
  ...
  if (typeof assignmentId !== "string" || !/^[0-9a-f-]{36}$/i.test(assignmentId)) throw new Error("Neplatné hodnotenie");
  if (!Number.isInteger(score) || score < 0 || score > 10) throw new Error("Skóre musí byť celé číslo od 0 do 10");
  ...
  if (error) throw new Error("Toto hodnotenie už bolo odoslané alebo vám nepatrí.");
