...
export async function confirmInvite(
  ...
  supabase: InviteAuthClient,
  status?: number,
) {
  if (!tokenHash) return NextResponse.redirect(new URL("/login?error=invite", request.url), status);

  const { error } = await supabase.auth.verifyOtp({ token_hash: tokenHash, type: "invite" });
  return NextResponse.redirect(
    new URL(error ? "/login?error=invite" : "/set-password", request.url),
    status,
  );
}
...
export async function GET(request: NextRequest) {
  ...
  return confirmInvite(
    ...
    supabase as unknown as InviteAuthClient,
  );
}
