...
export interface ApplicationStoreClient {
	from(table: "applications"): {
		insert(values: { applicant_name: string; form_data: StoredApplicationPayload["formData"]; delivery_status: "pending" }): {
			select(columns: "id"): { single(): Promise<InsertResult> };
		};
		update(values: { delivery_status: "sent" | "failed"; delivery_error: string | null; email_sent_at?: string }): {
...
export function createApplicationRepository(supabase: ApplicationStoreClient): ApplicationRepository {
	return {
		async createPending(data) {
			...
			throwIfError(error);
			if (!row) {
				throw new Error("Supabase did not return an application id");
			}
			return row;
		},
