...
export interface AttachmentClient {
  ...
  from(table: "application_attachments"): {
    ...
    select(columns: "storage_path"): {
      eq(column: "application_id", value: string): Promise<{
...
export interface ApplicationAttachmentRepository {
  store(
    ...
    data: Pick<ValidatedApplication, "cv" | "motivationLetter">,
  ): Promise<void>;
...
export function createApplicationAttachmentRepository(
  ...
  return {
    async store(applicationId, data) {
      ...
      if (data.cv) files.push({ kind: "cv", file: data.cv });
      ...
      try {
        for (const { kind, file } of files) {
          ...
          const { error: uploadError } = await bucket.upload(
            storagePath,
            details.bytes,
            { contentType: details.mimeType, upsert: false },
          );
