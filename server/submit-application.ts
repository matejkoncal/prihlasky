...
import { createApplicantConfirmationEmail, createSchoolEmail } from "./application-emails";
import { toStoredApplicationPayload, type PdfApplicationData } from "./application-types";
...
export async function submitApplication(input: unknown, dependencies: SubmissionDependencies): Promise<SubmissionResult> {
	const validation = validateApplication(input);
	if (!validation.success) {
		return validation;
	}

	const application = await dependencies.applications.createPending(toStoredApplicationPayload(validation.data));
