...
function parseAttachment(value: unknown): { attachment?: FileAttachment; decodedSize: number; error?: string } {
	...
	if (name && (name.length > 200 || UNSAFE_ATTACHMENT_NAME_PATTERN.test(name))) {
	...
	if (typeof content !== "string" || content.length === 0 || content.length % 4 !== 0 || !BASE64_PATTERN.test(content)) {
		return { decodedSize: 0, error: "Neplatný obsah prílohy" };
	}
...
export function validateApplication(input: unknown): ValidationResult {
	...
	const className = submittedClassName ?? (separatorIndex >= 0 ? submittedClassField!.slice(0, separatorIndex).trim() : submittedClassField);
	const fieldOfStudy = submittedFieldOfStudy ?? (separatorIndex >= 0 ? submittedClassField!.slice(separatorIndex + separator.length).trim() : "");
	const classField = submittedClassName && submittedFieldOfStudy ? `${submittedClassName}${separator}${submittedFieldOfStudy}` : submittedClassField;
	...
	if (typeof input.studentSituation !== "string" || !STUDENT_SITUATIONS.includes(input.studentSituation as StudentSituation)) {
	...
	if (input.personalDataConsent !== true) {
		return {
			success: false,
			error: "Na odoslanie prihlášky je potrebný súhlas so spracovaním osobných údajov",
		};
	}
	...
	if (cv.error) {
		return { success: false, error: cv.error };
	}
	...
	const data: ValidatedApplication = {
		...
		...(motivationLetter.attachment ? { motivationLetter: motivationLetter.attachment } : {}),
	};
