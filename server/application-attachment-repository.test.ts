...
function fakeClient(paths: string[] = []) {
	...
	const selectEq = vi.fn(async () => ({
		data: paths.map(storagePath => ({ storage_path: storagePath })),
		error: null,
	}));
...
describe("application attachment repository", () => {
	...
	it("uploads CV and motivation letter under random safe paths and records metadata", async () => {
		...
		expect(fake.upload).toHaveBeenNthCalledWith(1, `${applicationId}/cv-aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa.pdf`, expect.any(Uint8Array), {
	...
	it("removes stored objects and metadata", async () => {
		...
		expect(fake.remove).toHaveBeenCalledWith([`${applicationId}/cv.pdf`, `${applicationId}/letter.docx`]);
