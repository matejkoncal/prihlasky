...
export function createSchoolEmail(data: ValidatedApplication, pdfBase64: string): EmailMessage {
	const attachments: EmailAttachment[] = [{ filename: pdfFilename(data.name), content: pdfBase64 }];
	if (data.cv) {
		attachments.push(attachment(data.cv));
	}
